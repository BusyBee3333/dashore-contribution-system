...
registerAppTool(server, 'contribution_leaderboard', {
    ...
    try {
        ...
        LIMIT ?
      `).all(limit);
        const activeSeason = db.prepare('SELECT name FROM seasons WHERE active = 1 ORDER BY id DESC LIMIT 1').get();
        ...
        const textFallback = [
            ...
            '',
            entries.map(e => `${e.rank}. ${e.username} — ${type === 'season' ? e.season_points : e.total_points} pts | ${e.level_emoji} ${e.level_name}`).join('\n'),
        ].join('\n');
        return {
...
registerAppTool(server, 'contribution_profile', {
    ...
    try {
      ...
      `).get(username, username);
        if (!member) {
            ...
            return {
                content: [{ type: 'text', text: `Member "${username}" not found.` }],
                structuredContent,
            };
        }
...
registerAppTool(server, 'contribution_stats', {
    ...
    const db = openDb();
    try {
        const members = db.prepare('SELECT COUNT(*) as cnt FROM members WHERE total_points > 0').get().cnt;
        const contributions = db.prepare('SELECT COUNT(*) as cnt FROM contributions').get().cnt;
        const totalPoints = db.prepare('SELECT COALESCE(SUM(points), 0) as total FROM contributions').get().total;
        const vouches = db.prepare('SELECT COUNT(*) as cnt FROM vouches').get().cnt;
        const analysisRuns = db.prepare('SELECT COUNT(*) as cnt FROM analysis_runs').get().cnt;
        const activeSeason = db.prepare('SELECT * FROM seasons WHERE active = 1 ORDER BY id DESC LIMIT 1').get();
      ...
      `).all();
        ...
        if (activeSeason) {
            const seasonContribs = db.prepare('SELECT COUNT(*) as cnt, COALESCE(SUM(points), 0) as pts FROM contributions WHERE season_id = ?').get(activeSeason.id);
            seasonStats = {
        ...
        const textFallback = [
            ...
            '',
            '## Top Contributors',
            topContributors.map((c, i) => `  ${i + 1}. ${c.display_name || c.username} — ${c.total_points} pts (${c.level_name})`).join('\n'),
        ].join('\n');
        return {
            content: [{ type: 'text', text: textFallback }],
            structuredContent,
...
registerAppTool(server, 'github_contributions', {
    ...
    try {
      ...
      `;
        ...
        const recentEvents = db.prepare(eventsQuery).all(...eventParams);
      ...
      `).get();
        ...
        const textFallback = [
            ...
            '',
            '## Contributors',
            summary.map(s => `  @${s.github_author}: ${s.prs_merged} PRs, ${s.pr_reviews} reviews, ${s.bug_reports} reports → ${s.total_points} pts`).join('\n'),
            '',
            '## Recent Events',
            recentEvents.slice(0, 10).map(e => `  [${e.created_at.slice(0, 10)}] ${e.event_type} by @${e.github_author} on ${e.repo} (+${e.points_awarded}pts)`).join('\n'),
        ].join('\n');
        return {
            content: [{ type: 'text', text: textFallback }],
            structuredContent,
...
// ── UI Resource ───────────────────────────────────────────────────────────────
registerAppResource(server, 'Contribution Dashboard UI', UI_RESOURCE_URI, {
